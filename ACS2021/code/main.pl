...
abc:-
    ...
    fileName('ee', TheoryFile, Fnameee),
    open(Fnameee, write, StreamEE),
    assert(spec(repEE(StreamEE))),
...
repInsInc(TheoryStateIn, Layer, FaultStateIn, TheoryRep):-
    ...
    % calculate the entrenchment of axioms.
    entrechmentA(TheoryIn, FaultStateIn, [], EntState),
    writeAll(StreamEE, EntState), nl(StreamEE), nl(StreamEE),
    writeLog([nl, write_term('Axiom entrenchments: '), write_termAll(EntState),nl, finishLog]),
    /*
